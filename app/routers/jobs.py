...
@router.get(
    ...
    description=(
        ...
        "`is_active` defaults to true, so the common case does not have to remember to "
        "exclude closed jobs. Pass `is_active=false` for closed and expired postings, or "
        "omit it entirely with `is_active=null` to get everything.\n\n"
